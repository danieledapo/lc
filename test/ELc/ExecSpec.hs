...
runAll allElcs = evalState (go allElcs) emptyExecEnv
  where
    ...
    go [] = return []
    go (elc:elcs) = do
        val <- exec naiveInterpreter elc
        vals <- go elcs
        return $ val : vals

runWithLets :: [Let] -> ELc -> Lc
runWithLets lets elc = evalState (exec naiveInterpreter elc) (ExecEnv lets)

spec :: Spec
spec =
  describe "exec" $ do
    it "always evaluates the value of a let" $
      property prop_LetEvaluesToValue

    it "correctly evaluates some examples" $
      mapM_
...
prop_LetEvaluesToValue let_@(Let _ value) =
  eval naiveInterpreter value == run (ELcLet let_)
