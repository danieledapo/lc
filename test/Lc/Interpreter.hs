...
interpreterSpec interpreter =
    context "betaReduce" $ do
        it "correctly betaReduce I" $
            betaReduceAll' (appLcId (LcVar "a")) `shouldBe` [LcVar "a"]
                    ...
                    (LcApp
                       (LcAbs
                          ...
                          (LcAbs
                             "z"
                             (LcApp
                                (LcApp (LcVar "a") (LcVar "z"))
                                (LcApp (LcVar "y") (LcVar "z")))))
                       (LcVar "b"))
                    (LcVar "c")
                , LcApp
                    (LcAbs
        ...
        it "correctly eval that SKK is I" $
            eval' (appLcS lcK lcK (LcVar "ski")) `shouldBe` LcVar "ski"
